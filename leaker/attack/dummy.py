...
from ..api import KeywordAttack, KeywordQueryAttack, Dataset, LeakagePattern, RangeAttack, QuerySequence
...
class DummmyKeywordQueryAttack(KeywordQueryAttack):
    @classmethod
    def name(cls) -> str:
        return "Dummy"
    ...
    def recover(self, queries: List[int]) -> List[str]:
        with_random = random() % 10
        correct = ceil(len(queries) * with_random * 0.1)

        return queries[:correct] + (['dummy'] * (len(queries) - correct))
