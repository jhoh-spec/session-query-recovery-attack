...
from typing import Set, Iterator, Optional, List, Tuple, TypeVar, Type, Any, Dict
...
class WhooshKeywordQueryLog(KeywordQueryLog):
    ...
    def queries_with_times(self, user_id: str) -> List[Tuple[str, str]]:
        """
        Return [(query_content, query_time), ...] sorted by query_time for a user.
        Used by session-based attacks that need temporal information.
        Documents without a query_time field are skipped.
        """
        query = self.__query(user_id)
        if self.__searcher is None:
            with self.__index.searcher(closereader=False) as s:
                hits = list(self.__results(s, query))
        else:
            hits = list(self.__results(self.__searcher, query))
