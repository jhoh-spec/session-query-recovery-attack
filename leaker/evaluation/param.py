...
class KeywordQueryScenario(Enum):
    OTHER_USERS = 1   # attack user using log of other users as knowledge
    ALL_USERS = 2  # attack user using log of all users as knowledge
    EXACT = 3  # attack user using log of user under attack
    SPLIT = 4  # attack user sequence, taking one training set (first half) and one testing set (second half)
    SPLIT_SUB_KNOWN_DIST = 5  # attack user, taking one training set (first half) and one testing set (second half).
                             # The attacker gets a transition matrix. It is ensured that the test set trans matrix only
                              # contains keywords/states that are in the attacker knowledge.
    ARTIFICIAL = 6  # attack artificial distribution
    ARTIFICIAL_KNOWN_DIST = 7  # attack artificial distribution (adv knows transition matrix)
...
class EvaluationCase:
    ...
    dataset: Union[Dataset, RangeDatabase, KeywordQueryLog]
        the data set to evaluate the attacks on. For Sequence attacks, if no KeywordQueryLog is provided, it will be
        drawn from the transition matrix of the selected distribution.
    runs: int
    ...
    scenario: KeywordQueryScenario
        The scenario for KeywordQueryAttack evaluations
        default: ARTIFICIAL
    ...
    __scenario: KeywordQueryScenario

    def __init__(self, attacks: Union[AttackDefinition, Type[Attack], Iterable[Union[AttackDefinition, Type[Attack]]]],
                 dataset: Union[Dataset, RangeDatabase, KeywordQueryLog] = None, runs: int = 1, error: Type[Error] = None,
                 ...
                 scenario: KeywordQueryScenario = KeywordQueryScenario.ARTIFICIAL, pickle_description: str = None):
    ...
    def full_dataset(self) -> Union[Dataset, KeywordQueryLog]:
    ...
    def scenario(self) -> KeywordQueryScenario:
