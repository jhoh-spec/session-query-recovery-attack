...
class TotalVolume(LeakagePattern[int]):
    ...
    def leak(self, keywords: Iterable[str], dataset: Dataset) \
            -> List[int]:
        if dataset.has_extension(VolumeExtension):
            volume = dataset.get_extension(VolumeExtension)
            return [volume.total_volume(q) for q in keywords]
        else:
            return [sum(map(lambda doc: doc.length(), dataset(q))) for q in keywords]
