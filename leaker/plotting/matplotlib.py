...
class RangeMatPlotLibSink(MatPlotLibSink):
    ...
    log_y : Optional[bool]
        ...
        default: False
    use_mean : Optional[bool]
        whether to plot the mean of errors (True) or the median (False).
        default: True
    """
    ...
    def __init__(self, out_file: Optional[str] = None, markers: Optional[List[str]] = None,
                 log_y: Optional[bool] = False, use_mean: Optional[bool] = True):

        super().__init__(out_file, markers)
        self.__log_y = log_y
        self.__use_mean = use_mean
    ...
    def flush(self) -> None:
        ...
        for x, y, series_id, y_min, y_max, marker in self.yield_plotpoints(use_mean=self.__use_mean):
